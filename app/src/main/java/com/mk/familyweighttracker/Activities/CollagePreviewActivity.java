...
public class CollagePreviewActivity extends TrackerBaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        long userId = getIntent().getLongExtra(Constants.ExtraArg.USER_ID, -1);
        int collageTemplateId = getIntent().getIntExtra(Constants.ExtraArg.COLLAGE_TEMPLATE_ID, -1);

        setImageView(userId, collageTemplateId);
    }

    private void setImageView(long userId, int collageTemplateId) {
        if(userId == -1 || collageTemplateId == -1) return;


        CollageTemplateModel template = CollageTemplateModel.get(collageTemplateId);
        Bitmap resultBitmap = Bitmap.createBitmap(template.Width, template.Height, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(resultBitmap);
        Paint paint = new Paint();

        List<UserReading> userReadings = new UserService().get(userId).getReadings(true);
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            for (int r = 0; r < userReadings.size(); r++) {
                if(item.Sequence == userReadings.get(r).Sequence) {
                    Bitmap bitmap = userReadings.get(r).getImageAsBitmap(true, ImageUtility.SixHundred, ImageUtility.EightHundred);
